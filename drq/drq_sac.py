...
class DrQSAC(SAC):
  ...
  def __init__(
    ...
    if int(drq_num_views) != 2:
      raise ValueError(
          f"DrQ-v1 expects exactly 2 augmented views, got {drq_num_views}.")
    # Ensure the image key exists and is consistent with the replay buffer's expected key.
    if str(drq_image_key) != _DRQ_IMAGE_KEY:
      raise ValueError(
          f"DrQSAC only supports image key '{_DRQ_IMAGE_KEY}', got '{drq_image_key}'.")
  ...
  def _setup_model(self) -> None:
    ...
    if self.drq_image_key not in self.observation_space.spaces:
      raise KeyError(
          f"DrQSAC expected image key '{self.drq_image_key}' in observation space keys "
  ...
  def _sample_drq_views(
    ...
    if not isinstance(self.replay_buffer, DrQDictReplayBuffer):
      raise TypeError(
          "DrQSAC requires DrQDictReplayBuffer for DrQ two-view sampling.")
  ...
  def train(self, gradient_steps: int, batch_size: int = 64) -> None:
    ...
    for gradient_step in range(gradient_steps):
      ...
      current_q_values_a = self.critic(replay_view_a.observations,
                                       replay_data.actions)
      current_q_values_b = self.critic(replay_view_b.observations,
                                       replay_data.actions)
      ...
      q_values_pi = th.cat(
          self.critic(replay_view_a.observations, actions_pi), dim=1)
