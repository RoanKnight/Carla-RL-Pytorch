...
_OBS_KEYS = ('goal', 'traffic_light', 'distance_to_stop', 'speed', 'target_speed', 'speed_error',
             'last_action', 'lane_error_signed')
_SCALAR_OBS = {
  'distance_to_stop': ('distance_to_stop', 'm', True, 1,
                       lambda e: max(float(getattr(e, 'traffic_light_context_range_m', 1.0)), 1.0)),
  'speed':
  ('speed', 'kmh', False, 1, lambda e: max(float(e._get_speed_observation_clip_kmh()), 1.0)),
  'target_speed': ('reward_target_speed_kmh', 'kmh', False, 1,
                   lambda e: max(float(e._get_speed_observation_clip_kmh()), 1.0)),
  'speed_error': ('speed_error_kmh', 'kmh', True, 1,
                  lambda e: max(float(e._get_speed_observation_clip_kmh()), 1.0)),
  'lane_error_signed':
  ('lane_error_signed', 'm', True, 2,
   lambda e: max(float(e.config.get('lane_detection', {}).get('max_lane_deviation', 5.0)), 1e-6)),
}
...
def _format_obs_value(value) -> str:
  ...
  return repr(array.item()) if array.ndim == 0 else np.array2string(
    array, precision=3, floatmode='maxprec_equal')

def _format_obs_annotation(key: str,
                           value,
  ...
  if key in _SCALAR_OBS:
    ...
    if state_key not in state and (env is None or vector.size == 0):
      return ''
    real_value = float(
      state[state_key]) if state_key in state else float(vector[0]) * float(scale_getter(env))
...
def _format_observation(obs: dict,
                        env: Optional[CarlaEnv] = None,
                        state: Optional[dict] = None) -> str:
  if obs is None:
    return 'obs=<none>'
  ...
  if 'last_action' not in obs and 'prev_action' in obs:
    ...
    parts.append(
      f"prev_action={_format_obs_value(obs['prev_action'])}{' ' + annotation if annotation else ''}"
    )

  if 'image' in obs:
    image = np.asarray(obs['image'])
    parts.append(
      f"image=shape{image.shape}, dtype={image.dtype}, min={int(image.min())}, max={int(image.max())}"
    )
...
def _save_drq_image(episode: int, env: CarlaEnv, config: dict) -> bool:
  ...
  shifted = RandomShiftAug(pad=config['drq_pad'])(image_tensor).squeeze(0).permute(
...
def _log_route_stop_waypoint(env: CarlaEnv, state: dict, prefix: str) -> None:
  ...
  if tracked_stop_waypoint is None:
    ...
    return
  logging.info("%s: tl_id=%s state=%s dist=%5.1fm", prefix,
               tracked_tl_id if tracked_tl_id >= 0 else "none",
               state.get('traffic_light_state', 'none'), state.get('distance_to_stop', 999.0))
...
def _draw_route_stop_waypoint(env: CarlaEnv) -> None:
  ...
  if tracked_stop_waypoint is not None:
    ...
    env.world.debug.draw_line(stop_location,
                              stop_location + carla.Location(z=1.5),
                              thickness=0.20,
                              color=carla.Color(255, 255, 0),
                              life_time=0.1)

def _log_step(step: int,
              state: dict,
              reward: float,
              action: np.ndarray,
              obs: dict,
              env: Optional[CarlaEnv] = None) -> None:
  logging.debug(
...
def _print_waypoint_vectors(env: CarlaEnv) -> None:
  ...
  logging.info("Waypoint vectors (first %d starting from WP %d of %d):",
               min(10,
                   len(env.route) - start_idx), start_idx, len(env.route))
  ...
  for idx in range(start_idx, min(start_idx + 10, len(env.route))):
    ...
    logging.info("  WP %2d: (%+7.2f fwd, %+7.2f lat)", idx, dx * forward_x + dy * forward_y,
                 dx * right_x + dy * right_y)

def _update_transition_logs(step: int, info: dict, env: CarlaEnv,
                            prev_waypoint_timeout_active: bool, prev_tl_state: str,
                            prev_tracked_tl_id: int):
  ...
  if current_waypoint_timeout_active != prev_waypoint_timeout_active and current_waypoint_timeout_active:
    ...
    logging.info("Step %4d: waypoint-timeout tracking cleared (dist_to_wp=%.1fm)", step,
                 float(info.get('waypoint_distance', 0.0)))

  if current_tracked_tl_id != prev_tracked_tl_id:
    logging.info(
      "Step %4d: tracked_tl %s -> %s (state=%s, dist=%.1fm)",
      step,
      prev_tracked_tl_id if prev_tracked_tl_id >= 0 else "none",
      current_tracked_tl_id if current_tracked_tl_id >= 0 else "none",
      current_tl_state,
      float(info.get('distance_to_stop', 999.0)),
    )
  if current_tl_state != prev_tl_state:
    logging.info(
      "Step %4d: traffic_light %s -> %s at %.1fm (tl_id=%s)",
      step,
      prev_tl_state,
      current_tl_state,
...
def run(config: Optional[dict] = None) -> None:
  ...
  logging.info("map=%s weather=%s steps=%s episodes=%s control_mode=%s", debug_config['map'],
               debug_config['weather'], debug_config['max_steps'], debug_config['episodes'],
               control_mode)
  logging.info('=' * 70)
  ...
  env.phase_config['distribution'] = {
    'maps': [debug_config['map']],
    'weathers': [debug_config['weather']]
  }
  ...
  logging.debug("Overrides: map=%s weather=%s mode=%s traffic_lights_override=%s",
                debug_config['map'], debug_config['weather'], control_mode,
                debug_config['traffic_lights_enabled_override'])
  ...
  if control_mode == 'rl':
    ...
    if model_path is None:
      ...
      if model_path is None:
        raise FileNotFoundError("control_mode='rl' but no checkpoint found in "
                                f"{checkpoint_dir}/")
  ...
  try:
    for episode in range(1, debug_config['episodes'] + 1):
      ...
      logging.info("Reset: map=%s weather=%s dist=%.1fm traffic_lights_enabled=%s", info['map'],
                   info['weather'], info['initial_distance'],
      ...
      step = 0
      pending_camera_snapshot = debug_config['save_image_on_first_step'] and getattr(
        env, 'camera_enabled', False)
      pending_drq_snapshot = debug_config['save_drq_image_on_first_step'] and getattr(
        env, 'camera_enabled', False)
      ...
      while not done:
        action = np.asarray(agent.predict(obs, deterministic=True)[0],
                            dtype=np.float32) if agent is not None else np.array([0.0, 0.0],
                                                                                 dtype=np.float32)
        ...
        if (pending_camera_snapshot or pending_drq_snapshot) and env._rgb_image is not None:
          pending_camera_snapshot = pending_camera_snapshot and not _save_camera_image(
            episode, env, debug_config)
          pending_drq_snapshot = pending_drq_snapshot and not _save_drq_image(
            episode, env, debug_config)
        elif (pending_camera_snapshot
              or pending_drq_snapshot) and step >= debug_config['camera_snapshot_max_wait_steps']:
          logging.warning(
        ...
        if spectator is not None and control_mode == 'rl':
          ...
          location = vehicle_transform.location - forward * \
              debug_config['spectator_distance_behind'] + \
